...
WORKDIR은 mkdir과 접속을 합쳐놓은 것이라고 생각하면 편할것같다.
그 다음에 RUN을 넣어서 아까 만들어둔 /var/www/html에 index.html 파일을 넣어준다.
대망의 CMD는 실행될때 사용되는 명령어로 여기선 python3의 http.server 라는 기능을 사용한다는 의미이다.
